...
class PersonTracker:
    def __init__(self):
        ...
        self.trackers = {
            'camera1': BYTETracker(
                ...
                track_buffer=30,   # Frames to keep dead tracks
        ...
        # Tracking parameters
        self.reid_threshold = 0.75  # Increased threshold for stricter matching
    ...
    def format_detections(self, detections):
        """Format detections for ByteTracker input
        Args:
            detections: numpy array of detections in format [x1,y1,x2,y2,conf,cls]
        Returns:
            Tuple of (bboxes, scores, classes) as separate arrays
        """
        if len(detections) == 0:
    ...
    def process_frame(self, frame, camera_id, timestamp):
        """Process frame using ByteTracker"""
        # Run YOLO detection
        results = self.yolo_model(frame)
        
        # Convert YOLO results to detection format
        detections = []
        for det in results[0].boxes.data:            # Keep as tensor, don't convert to numpy yet
            x1, y1, x2, y2, conf, cls = det
            if int(cls) == 0:  # person class
                # Format: [x1, y1, x2, y2, score, class]
                detections.append(torch.tensor([x1, y1, x2, y2, conf, 0]))
        
        if not detections:
            return frame
            
        # Stack tensors and keep as torch tensor
        ...
        if len(detections) > 0:
            try:
                online_targets = self.trackers[camera_id].update(
                    detections,  # Pass tensor directly
                    [frame.shape[0], frame.shape[1]]  # img_size
                )
                
                # Process each tracked target
                for track in online_targets:
                    # The track object is now a numpy array with format [x1, y1, x2, y2, score, id]
                    if len(track) >= 6:  # Make sure we have all required values
                        x1, y1, x2, y2, score, track_id = track[:6]
                        ...
                        if self.is_in_door_area(bbox, camera_id):
                            ...
                            if features is not None:
                                # Match with existing tracks using ReID
                                matched_id = self.match_person(features, timestamp, camera_id)
                                
                                if matched_id is None:
                                    matched_id = len(self.tracked_individuals)
                                
                                # Update tracking info
                                self.update_person_info(
                                    matched_id, frame, bbox, camera_id, timestamp, features)
                                
                                # Draw tracking visualization
                                if matched_id not in self.completed_tracks:
                ...
                if len(online_targets) > 0:
                    print(f"Track example: {online_targets[0]}")
    ...
    def reset_tracking(self):
        """Reset tracking states and ByteTracker"""
