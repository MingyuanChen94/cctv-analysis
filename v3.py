...
class PersonTracker: 
    def __init__(self):
        ...
        self.counting_zones = {
            'camera1': [(1030, 200), (1700, 300)],  # Adjust coordinates as needed
            'camera2': [(400, 200), (800, 300)]     # Adjust coordinates as needed
        }
        ...
        self.tracker = BYTETracker(
            ...
            track_buffer=30,   # Frames to keep track alive without detection
        ...
        self.camera_appearances = defaultdict(dict)  # {person_id: {camera_id: (first_time, last_time)}}
        ...
        self.max_track_gap = 1.0    # Maximum time gap for track continuation (seconds)
    ...
    def extract_reid_features(self, frame, bbox):
        ...
        try:
            ...
            person_img = cv2.cvtColor(person_img, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
                ...
                if self.is_in_door_area(bbox, camera_id):
                    ...
                    if features is not None:
                        ...
                        if person_id not in self.current_frame_detections:
                            ...
                            if len(person_info.prev_positions) >= 7:  # Need more positions for sequence validation
                                if not self.validate_movement_sequence(person_info, bbox):  # Fixed: Pass both arguments
    ...
    def validate_movement_sequence(self, person_info, current_bbox):
    ...
    def validate_track_continuity(self, person_info, current_time):
        ...
        for i in range(1, len(person_info.prev_positions)):
            gap = person_info.prev_positions[i][1] - person_info.prev_positions[i-1][1]
    ...
    def validate_intercamera_timing(self, first_camera_time, second_camera_time):
        ...
        MAX_TRANSIT_TIME = 180  # 3 minutes maximum
    ...
    def track_between_cameras(self):
        ...
        for person_id, camera_info in self.camera_appearances.items():
            if 'camera1' in camera_info:
                camera1_tracks[person_id] = camera_info['camera1'][1]  # Use last appearance
            if 'camera2' in camera_info:
                camera2_tracks[person_id] = camera_info['camera2'][0]  # Use first appearance
    ...
    def analyze_trajectories(self, person_info, bbox, camera_id):
        ...
        avg_velocity = sum(y_movements) / len(y_movements)
        
        # Near top of door area (potential entry or exit)
        if current_y < door_top + door_height * 0.3:
            if avg_velocity > 2:  # Moving downward significantly
                return 'entering'
            elif avg_velocity < -2:  # Moving upward significantly
                return 'exiting'
    ...
    def extract_date_from_filename(self, filename):
        ...
        try:
            ...
            date_str = str(filename).split('_')[-1].split('.')[0]  # Get YYYYMMDD part
            return date_str
        except:
            return None
    ...
    def process_videos(self, video_dir, output_dir=None):
        ...
        for date, video_files in videos_by_date.items():
            ...
            for video_file in sorted(video_files):  # Sort to ensure Camera_1 processes first
    ...
    def save_tracking_data(self, output_dir, date):
        ...
        with open(transitions_file, 'w', newline='') as f:
            ...
            for person_id in self.camera1_to_camera2:
                ...
                if person_info:
                    camera1_times = person_info.camera_times.get('camera1', {'first': None, 'last': None})
                    camera2_times = person_info.camera_times.get('camera2', {'first': None, 'last': None})
                    
                    if camera1_times['last'] is not None and camera2_times['first'] is not None:
                        transit_time = camera2_times['first'] - camera1_times['last']
                        writer.writerow([
                            date,
                            person_id,
...
class PersonInfo:
    def __init__(self, person_id):
        ...
        self.appearances = []
        self.features = []  # Store multiple features for better matching
        self.prev_positions = []  # Store previous positions for movement tracking
        self.entry_position = None  # Store entry position
        self.state = 'entering'  # Only track entering people
        ...
        self.last_seen = None
        self.counted = False  # Flag to prevent double counting
        self.track_started = False  # Flag to indicate if tracking has started
        self.track_confidence = 0  # Confidence in track (increments with consistent detections)
        self.last_position = None  # Last known position
        self.counting_zone_entered = False  # Flag for entering counting zone
        self.counting_zone_exited = False   # Flag for exiting counting zone
        self.track_complete = False  # Flag to indicate if person has been fully tracked
        self.entry_recorded = False
        self.has_exited_camera1 = False
        self.camera1_exit_time = None
        self.camera_times = {}  # Store first/last times for each camera
